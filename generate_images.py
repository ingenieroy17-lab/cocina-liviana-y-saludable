...
def main():
    ...
    md_path = r"r:\ROY\FACULTAD Y TRABAJOS\EMPRENDIMIENTOS\IA\Antigravity Projects\RECETARIO SALUDABLE\prompts-bono3.md"
    output_dir = r"R:\ROY\FACULTAD Y TRABAJOS\EMPRENDIMIENTOS\IA\Antigravity Projects\RECETARIO SALUDABLE\assets\images\imagenes dentro de libros\bueno3 - Recetas sin TACC - fotos"
    
    os.makedirs(output_dir, exist_ok=True)
    
    recipes = parse_prompts(md_path)
    # Filter to only recipes 1 to 20
    recipes_to_run = [r for r in recipes if r["num"] >= 1]
    
    print(f"Loaded {len(recipes)} recipes from markdown. Filtered to run {len(recipes_to_run)} recipes (1 to 20).")
